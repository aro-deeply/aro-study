...
- 인증(2026-09-10 변경): 멤버는 비밀번호 없음. 처음 열면 Firebase **익명 로그인**(기기 토큰, 화면에 보이지 않음)을 자동으로 하고 그 기기에 유지한다(콘솔에서 "익명" 제공업체를 켠다). 그다음 "이름 선택" 드롭다운(멤버 명단은 Firestore `members`에서 읽음)으로 작성자를 정하고 localStorage에 기억한다. **총무만** admin.html에서 총무 계정(이메일/비밀번호, `ADMIN_EMAIL`, 기기당 한 번)으로 로그인하며, 관리 데이터 쓰기는 Firestore 규칙에서 이 계정만 허용한다. 읽기는 접속한 누구나 가능하므로 주소를 아는 사람은 볼 수 있다(사용자가 감수하기로 결정).
- Firestore 보안 규칙(`firestore.rules`): 읽기는 접속한 누구나(`request.auth != null`, 익명 포함). `members`·`settings`·`terms`·`dues`·`sessions`·`expenses`·`payments` 쓰기는 총무 계정(`request.auth.token.email == ADMIN_EMAIL`)만. `comments`·`annotations`·`resources`는 누구나 만들되 문서의 `uid`가 접속 uid와 같아야 하고, 수정·삭제는 같은 `uid`(본인) 또는 총무만. Storage도 같은 방식(파일 metadata.uid). 정적 HTML에는 민감한 내용을 두지 않는다(아래 3절 참조).
...
- 총무 계정 이메일(코드 상수 `ADMIN_EMAIL`, 규칙의 `isAdmin()`에도 같은 값): `aro.deeply@gmail.com`. 2026-09-10 이전에는 멤버 전원이 이 계정을 공용 비밀번호로 썼고, 이후에는 총무만 admin.html에서 쓴다.
...
- `settings/rotation`: `{ order: [memberId...], startMonth: "YYYY-MM" }`. 발제 순서. startMonth의 발제자가 order[0], 매월 한 칸씩. 끝난 뒤는 미정(반복하지 않음, 새 순서를 정하면 startMonth를 갱신). (2026-09-10 추가)
- `settings/notice`: `{ text, until: "YYYY-MM-DD" | "" }`. 총무가 쓰는 공지. text가 있고 until이 지나지 않았으면 첫 화면 상단 배너(`#note`)로 표시, 비우면 표시하지 않음. 입력은 admin 일정·발제 탭. (2026-09-10 추가)
...
- 회비 밖 추가 비용만 참석자 n분의 1. 규칙(2026-09-10 변경): 분배 대상이 같은 지출을 합쳐 인원으로 나누고 100원 단위로 올림. 올림으로 더 걷히는 차액은 회비로 적립된다(총무가 받는 금액에 포함되고 회비 수입으로 잡힌다). 추가 비용이 없으면 개인 정산 표는 표시하지 않는다.
...
### 4-1. 공통
- 첫 진입 시 익명 로그인 자동(화면 없음, 기기에 유지). 실패하면(콘솔에서 익명이 꺼짐, 네트워크) 이유와 "다시 시도" 버튼만 보인다. 그 다음 "이름 선택" 한 번(localStorage 기억, 상단에서 변경 가능). admin.html은 총무 이름을 고른 뒤 총무 비밀번호 한 칸(기기에 유지, "로그인 해제" 가능).
- 상단 바: 사이트명 `HR STUDY`(작게, 자간 넓게) / `모임 기록` (굵게). 오른쪽에 현재 이름(누르면 바꾸기), 총무면 "관리" 링크. 로그아웃은 없다.
- 모바일 우선. 560px 이하에서 단일 컬럼.
- PWA: manifest.json과 아이콘을 넣어 홈 화면 추가가 되게 한다(aro-briefs와 같은 방식).

### 4-2. index.html (목록, 2026-09-10 개편: 요약 대시보드 + 탭)
- 상단 요약 카드 4개: 다음 모임(세션이 있으면 확정 일정·시간·장소, 없으면 정기 규칙), 발제자, 회비 잔액(납부 n/m명), 이달 가용.
- 탭 4개(해시 라우팅 `#summary|#months/YYYY-MM|#fund|#schedule`): 요약(다음 모임 카드, 확인할 것: 미납·보전·남은 정산, 최근 모임 3개), 월별(달 선택 칩, 그 달의 모임·발제·회비 배정/이월/지출/남은 금액·지출 내역·납부·회차별 정산서), 회비(renderFund), 일정·발제(renderSchedule).
- 주차 카드: 날짜, 제목, 참석 인원, 발제자, 지출 합계, 브리프 수, 의견 개수. 오늘 이후이고 page가 없으면 "예정" 카드(시간·장소 표시).
...
3. 다음 모임 계획: 정적 HTML
4. 비용: `settle.js`의 `mountSettlement`가 이 세션의 expenses를 읽어 두 부분으로 렌더. (1) 회비 지출: 이 회차 합계, 현재 회비 잔액, 내역, 대신 결제분 보전 상태(`fund.js`). (2) 추가 비용(있을 때만): 아래 6절 형식. 항목 목록, 카테고리 합계, 1인 부담액(참석자 기준 균등, 100원 단위 올림, 차액은 회비 적립), 누가 얼마 냈고 누가 누구에게 얼마 보내야 하는지.
5. 의견: 댓글 목록 + 입력. 작성자는 선택한 이름. 삭제는 본인 글만(문서 `uid` = 접속 uid, 규칙에서 검사, 총무는 전부 가능).
2-1. 관련 자료(`resources.js`, 2026-09-10 추가, 화면 순서는 내용 다음): 제목·링크·메모를 누구나 올리는 목록. 링크는 https 보정, 본인 것만 삭제, 최신순.
6. 하이라이트·메모: 기존 reader.js 기능. "내 것만 보기 / 전체 보기" 토글 추가. "AI용 복사"는 전체 메모를 사람별로 묶어 텍스트로 만든다.

### 4-4. admin.html (총무용)
- 이름이 `총무` 권한(`members`에 `role: "admin"`)이고 총무 계정으로 로그인한 경우만 진입. 관리 컬렉션 쓰기는 규칙에서 총무 계정만 허용(2026-09-10).
...
- 총 지출(큰 숫자, 어두운 카드) + 오른쪽에 정산 기준(N인 균등, 100원 단위 올림, 나눗셈식, 회비 적립액)
- 1인 정산 금액 카드 3~N개 (총무 카드만 웜 톤 배경, "회비 보관" 태그와 적립액)
