...
- 인증(2026-09-10 변경): 비밀번호 없음. 처음 열면 Firebase **익명 로그인**을 자동으로 하고 그 기기에 유지한다(콘솔에서 "익명" 제공업체를 켠다). 그다음 "이름 선택" 드롭다운(멤버 명단은 Firestore `members`에서 읽음)으로 작성자를 정하고 localStorage에 기억한다. 주소를 아는 사람은 누구나 들어올 수 있다(사용자가 감수하기로 결정). 이전 방식(이메일/비밀번호 공용 계정)은 git 기록에 있다.
- Firestore 보안 규칙: 모든 컬렉션에 대해 `allow read, write: if request.auth != null;`. 익명 로그인도 통과하므로 실제 접근 제한은 아니고, SDK를 거치지 않은 직접 호출만 막는다. 정적 HTML에는 민감한 내용을 두지 않는다(아래 3절 참조).
...
- 공용 로그인 이메일 상수(`LOGIN_EMAIL`)는 2026-09-10 익명 로그인 전환으로 삭제했다. 콘솔에 남은 공용 계정은 사이트가 쓰지 않는다.
...
### 4-1. 공통
- 첫 진입 시 익명 로그인 자동(화면 없음, 기기에 유지). 실패하면(콘솔에서 익명이 꺼짐, 네트워크) 이유와 "다시 시도" 버튼만 보인다. 그 다음 "이름 선택" 한 번(localStorage 기억, 상단에서 변경 가능).
...
### 4-4. admin.html (총무용)
- 이름이 `총무` 권한(`members`에 `role: "admin"`)인 경우만 진입. 서버 검증은 하지 않는다(리스크 낮음, 사용자 판단).
