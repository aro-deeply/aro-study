...
- `index.html`: 다음 모임·발제자·회비 잔액·이달 가용 요약, 일정과 발제(12개월 표, 발제 순서), 회비 현황(회비 설정, 월별 가용 금액, 멤버별 납부 표, 보전할 돈), 주차 목록(최신순), 추가 비용 정산(회차별 정산서, 있을 때만).
...
- 추가 비용(재원 "참석자 n분의 1")만 개인 정산: 분배 대상이 같은 항목을 합쳐 균등, 1인 부담은 100원 단위 올림, 올림으로 더 걷히는 차액은 회비로 귀속(총무가 받아 회비 수입으로 잡힘). 차액은 낸 금액에서 부담액을 뺀 값이며 +는 받을 돈, -는 낼 돈. 송금 기록은 남은 차액에 반영된다. 검산(소계 합, 부담액 합 = 총액 + 귀속액, 중복)은 화면에서 실제로 수행한다.
