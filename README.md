...
- admin.html에서 멤버 등록(본인은 "총무" 체크), 회비 기수 만들기와 납부 처리, 모임 세션 생성, 지출 입력(영수증 보고 바로, 기본은 "회비에서"), 송금 기록.
- 모임 후 기록 페이지는 Claude Code에서 `.claude/skills/aro-study-week/SKILL.md` 절차로 만든다. 메모를 주면 `weeks/<날짜>/index.html`을 만들고 세션을 등록하고 푸시한다.

## 화면

- `index.html`: 회비 잔액·미납·다음 모임 요약, 회비 현황(기수, 멤버별 납부 표, 보전할 돈), 주차 목록(최신순), 추가 비용 정산(있을 때만).
...
- 기본: 기수마다 1인 회비를 걷어 총무가 보관하고, 모임 비용은 회비에서 쓴다. 잔액 = 납부 합계 - 회비 지출 합계. 회비 금액·기간·대상은 admin 회비 탭에서 바꿀 수 있다.
