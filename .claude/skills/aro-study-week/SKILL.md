...
- 다룬 브리프: aro-briefs 목록(`../aro-briefs/index.html`의 `.brief` 카드)에서 제목·URL을 찾는다. 없으면 생략.
...
- 다음 모임 계획(`--next`)에 날짜를 쓸 때는 정기 규칙(매월 둘째 화요일)으로 계산한 날짜인지 확인한다. 목록 화면의 "다음 모임"은 규칙으로 자동 계산되므로 계획 문구에는 준비물·장소 위주로 쓴다.
