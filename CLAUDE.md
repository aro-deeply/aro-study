...
- `assets/app.js` 인증·이름 선택·상단 바·Firestore 헬퍼·유틸 / `assets/fund.js` 회비(회비 설정·납부·잔액·보전) 계산·렌더 / `assets/settle.js` 추가 비용 n분의 1 정산 계산·렌더, 주차 페이지 비용 섹션 마운트 / `assets/reader.js` 하이라이트·메모·의견 / `assets/style.css` 공통 스타일(aro-briefs 톤).
