...
- 정산서 사람 카드 둘째 줄(2026-09-11): 송금 기록까지 반영한 현재 상태만 보여준다. "보낼 돈 N" / "받을 돈 N" / "완료 · 날짜". "낸 돈 N"은 직접 결제한 사람에게만 앞에 붙인다. 낸 돈 - 부담액 차액(송금 반영 전)은 더 표시하지 않는다.
- 2026-09-11 추가: 발제 주제(세션 title)는 모임 카드와 월별 탭의 발제 박스에만. 정산서 제목·요약 '정산 남음' 줄에는 날짜만. 월별 박스는 "날짜 (요일) 시간" 한 줄에 아래 장소만(시간을 혼자 두지 않는다). 회비 설정 박스는 기간·1인·대상·납부·지출·계좌를 `.kv` 행으로. 일정 탭 규칙 박스 둘째 줄은 "발제자 N명 · 순서는 상호 합의로 변경 가능". 카드의 "브리프 N편"은 없앴다(`briefCount`는 더 쓰지 않음). "최근 모임 N회"는 예정 카드를 세지 않는다.
