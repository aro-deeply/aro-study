...
- `assets/app.js` 인증·이름 선택·상단 바·Firestore 헬퍼·유틸 / `assets/schedule.js` 정기 모임 날짜(n번째 요일)·발제 순서 계산·렌더, `settings` 읽기 / `assets/fund.js` 회비(회비 설정·납부·잔액·보전·월별 가용 금액) 계산·렌더 / `assets/settle.js` 추가 비용 n분의 1 정산 계산·렌더, 주차 페이지 비용 섹션 마운트 / `assets/reader.js` 하이라이트·메모·의견 / `assets/style.css` 공통 스타일(aro-briefs 톤).
...
- 2026-09-10 추가: 정기 모임은 매월 둘째 화요일(`settings/meeting`), 발제는 `settings/rotation` 순서대로 매월 한 명(세션 `presenter`가 있으면 우선), 회비는 `terms.months`로 나눈 월별 가용 금액과 이월. SPEC 3-2.
